...
\begin{document}
	...
	You want to have multiple lines? Take a look at \hyperref[fig:multiple_line]{\textbf{Figure \ref*{fig:multiple_line}}}:
	...
	\hyperref[fig:scatter_pedram]{\textbf{Figure \ref*{fig:scatter_pedram}}}, we can see the scatter plot representing the relationship between X and Y.
	
	In this updated example, the \codebox{\textbackslash hyperref} command is used within the caption to create a hyperlink. You can use \codebox{\textbackslash protect}
	
	by
	
	\begin{lstlisting}
		caption{\protect\hyperref[fig:scatter]{Figure \ref*{fig:scatter}}: This is a scatter Plot}
	\end{lstlisting}
	
	command is used to ensure that the hyperlink is properly handled in the list of figures and other locations where the caption may be repeated. The label fig:scatter is assigned to the figure, and the \codebox{\textbackslash ref*} command is used to avoid generating additional clickable links within the sentence.
	
	By modifying the caption format in this way, the entire "Figure 1" text will function as a hyperlink to the corresponding plot.
